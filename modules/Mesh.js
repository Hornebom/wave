...
function Mesh({ 
  ...
  function bindBuffers() {
    ...
    buffers.push(
      new Buffer({ gl, extension, program, data: instanceOffsets, name: 'a_offset', size: 1, instancedAttribute: true }),
      new Buffer({ gl, program, data: vertices, name: 'a_vertex', size: 2 }),
      new Buffer({ gl, extension, program, data: indexes, name: 'a_index', size: 1, instancedAttribute: true })
    )
...
function getInstanceOffsets({ rectangleWidth, canvasWidth, count }) {
  ...
  const start = (rectangleWidth * .5 / canvasWidth) * clipSpaceRange 
